...
var connection = mysql.createConnection({
    ...
    password: "fun2code", //Supply your password here
    database: "bamazonDB"
})
...
function allItemSearch(){
    var query = "SELECT item_id, product_name, department_id, department_names,price, stock_quantity FROM products where stock_quantity <> 0";
    //Open database connection
    //connectDB();
    connection.query(query,function(err, res){
        var table = new Table({
            head: ['item_id', 'product_Name', 'department_id', 'department_names', 'price','stock_quantity']
            , colWidths: [10,50,10,35,10,30]
        });
   
        for(var i=0; i < res.length; i++){
            table.push(
                [res[i].item_id,res[i].product_name,res[i].department_id,res[i].department_names,res[i].price,res[i].stock_quantity]        
            );
...
function checkInventory(){
    var query = "SELECT item_id, product_name,department_id, department_names, price, stock_quantity FROM products where stock_quantity <= 5";
    //Open database connection
    connection.query(query,function(err, res){
...
function addNewProduct(){
    inquirer.prompt([
        ...
        },
        {
            name: "department_id",
            type: "input",
            message: "Enter department ID for the new product ",
        },
        {
            name: "department_name",
            type: "input",
            message: "Enter department for the new product ",
        },        
        {
            name: "price",
            type: "input",
        ...
        connection.query(
            "INSERT INTO products SET ?",
            {
              ...
              department_names: answer.department_name,
              price: answer.price,
